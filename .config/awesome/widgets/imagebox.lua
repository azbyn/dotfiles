...
function imagebox:draw(_, cr, width, height)
    ...
    cr:mask(pat.create_for_surface(self._private.image), 0, 0)
...
function imagebox:set_color(color, no_redraw)
    self._private.color = parse_color(color)
    if no_redraw then return end
...
function imagebox:set_image(image, color)
    ...
    self:set_color(color or {1, 1, 1, 1}, true)
...
local function new(image, color, resize_allowed, clip_shape)
    local ret = base.make_widget(nil, nil, {enable_properties = true})

    gtable.crush(ret, imagebox, true)

    if image then
        ret:set_image(image, color)
    end
